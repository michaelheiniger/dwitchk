...
interface PlayerDao {
    ...
    fun insertNewGuestPlayer(gameLocalId: Long, name: String): Long {
        val player = Player(0,
            PlayerInGameId(0), gameLocalId, name, PlayerRole.GUEST, PlayerConnectionState.CONNECTED, false)
        val playerLocalId = insertPlayer(player)
        updatePlayer(player.copy(id = playerLocalId, inGameId = PlayerInGameId(
            playerLocalId
        )
        ))
    ...
    @Query("UPDATE Player SET connectionState=:state WHERE in_game_id=:playerInGameId")
    ...
    fun updatePlayerWithStateAndReady(playerInGameId: PlayerInGameId, state: PlayerConnectionState, ready: Boolean): Int
    ...
    @Query("SELECT Player.* FROM Player,Game WHERE Game.id=:gameLocalId AND Game.local_player_id=Player.id")
    ...
    @Query("""
        ...
        """)
    fun observeConnectedPlayers(gameLocalId: Long): Flowable<List<Player>>
